...
// 규율 래더 재설계 (점수 vmax) — npx tsx scripts/simDisciplineLadder.ts
//
// ── 왜 다시 짜는가 ────────────────────────────────────────────
...
//
// ── 표적을 모집단에 넣는다 ─────────────────────────────────────
// simSkillSeparation의 코호트는 전부 **기대 점수를 최대화**한다. vmax는 적정 점수법이라
...
const LADDERS: Ladder[] = [
  ...
  // vmax 점수는 이미 로그우도비다: 카드의 정보량 합 D = ln( L(신고 모델) / L(무정보) ).
