...
// 판정 되돌리기:
//   npm run judgment:revert -- <judgmentId> <운영자 이메일> "사유"
...
async function main() {
  ...
  const [judgmentId, operatorEmail, reason] = args;
  if (!judgmentId || !operatorEmail || !reason) {
    console.error('사용법: npm run judgment:revert -- <judgmentId> <운영자 이메일> "사유"');
  ...
  try {
    ...
    console.log(`  카드 ${r.predictionCardId} 는 다음 배치가 다시 판정합니다`);
