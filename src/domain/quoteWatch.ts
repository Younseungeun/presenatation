 ...
 *
 * @근거 설계 하루 q 변동 최대 0.33(실측) — 중단선까지 하루 거리 안
 */
 ...
 *
 * @근거 설계 이력 현상 — 같은 값이면 문턱에서 등록·해제가 진동한다
 */
 ...
 *
 * @근거 설계 모르는 상태에서 상품을 지우지 않는다 (priceCache와 같은 태도)
 */
 ...
 * 다시 편입시킨다.
 *
 * @근거 설계 비용이 비대칭 — 잘못 풀어도 결제 관문이 막고 다시 편입시킨다
 */
...
/**
 * 감시 대상이 상한(60)을 넘어 "지연"이 **이만큼 연속되면** 운영자에게 알린다.
 *
 * 왜 6시간인가 — 이 메커니즘의 지연은 "몇 분 반짝했다 사라지는" 것이 아니다.
 * q는 한 틱(2분)에 ~0.02밖에 안 움직여, 감시 대상 수는 하루 사이 거의 변하지 않는다.
 * 실측(scripts/simQuoteWatchLoad.ts): 감시 대상이 60 언저리인 **경계** 규모에서는
 * 하루 한나절(최대 ~294분)까지 지연이 갔다가 다음 세션에 저절로 풀리고, 규모가
 * 상한을 확실히 넘으면 **세션 100%**(390분 내내) 지연이다. 6시간이면 경계의 하루짜리
 * 스파이크는 넘기고(오탐 0), 구조적 초과는 그 세션 안에 잡는다.
 *
 * 이 알람은 "장애"가 아니라 **"규모가 시세 예산을 넘어섰다 — 소스를 늘려라"**는
 * 용량 신호다. 결제 차단은 별도 실시간 조회라 그동안에도 돈은 새지 않는다.
 *
 * @근거 시뮬 scripts/simQuoteWatchLoad.ts — 경계 지연 최대 ~294분·자가해소, 구조적은 세션 100%
 */
export const SLOW_ALERT_AFTER_MS = 6 * 60 * 60_000;

/**
 * slow 관측이 이보다 오래 끊기면 **새 지연 구간**으로 본다.
 * 회차 몇 번 건너뜀(스케줄러 순간 지연)은 한 구간으로 견디고, 장 마감(약 17시간
 * 값이 안 변함)은 리셋해 "어제 지연이 오늘 개장 즉시 알람"이 되지 않게 한다.
 *
 * @근거 설계 2분 주기의 몇 배 — 일시 누락은 견디고 세션 경계는 리셋한다
 */
export const SLOW_GAP_RESET_MS = 12 * 60_000;
