...
// 시뮬레이션 근거 (scripts/simSkillSeparation.ts — 점수 vmax 재캘리브레이션, 2026-08-13):
//
// 점수 모델이 v4(공정배당 이항) → vmax(정보량 로그 점수)로 바뀌며 눈금이 통째로
...
//     — vmax는 적정 점수법이라 EV 최적이 곧 "정직하게 신고한다"와 같은 뜻이다
//
// 같은 시뮬이 분리력도 함께 쟀다 (등급이 실력을 가르는지는 피라미드 모양이 말해주지 않는다):
//   · **오분류 0%** — 스팸이 시니어에 오르지도, 정밀이 시니어에 못 오르지도 않는다
//   · AUC  스팸<준수 0.961 / 하위<준수 0.939 / 준수<우수 0.973 / 우수<정밀 0.983
//     (v4 대비: 하위<준수 0.785 → 0.939, 스팸<준수 0.819 → 0.961)
//   · 순위상관(실력↔점수) 0.860 (v4 0.792)
//   · 도달률: 준수 42% 시니어 / 우수 81% 마스터 / 정밀 98% 펠로우
//
// 2026-08-13 수익성 가중 w를 구간 계단 → 연속으로 편 뒤 다시 잡았다(scoring 참조).
// 점수 눈금이 약간 내려가(같은 카드의 w 평균이 낮아진다) 임계값도 함께 내려간다.
// 부수 효과로 **분리력이 좋아졌다** — 계단이 만들던 경계 뭉침이 사라졌기 때문이다
// (순위상관 0.835 → 0.860, AUC 준수<우수 0.936 → 0.973).
