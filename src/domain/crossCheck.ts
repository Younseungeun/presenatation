 ...
 *
 * ── 폐기된 조건 ③ — **처리 용량을 전환 조건에 넣으면 안 된다** (2026-08-15 철회) ──
 * 한때 "예상 불일치 건수 ≤ 운영자 처리 용량"을 세 번째 조건으로 뒀다. **안전 장치의
 * 방향을 정확히 거꾸로 세운 설계였다.**
 *
 * 시장 급변·거래소 API 개편·중계망 균열이 나면 불일치는 10건이 아니라 50~100건으로
 * 튄다. 그 조건대로면 **불일치가 폭증하는 바로 그날 enforce가 거부되고**, 신뢰가
 * 깨진 단일 소스로 자동 판정이 그대로 나간다 — 대조가 가장 필요한 날에 대조를 끄는 것이다.
 *
 * 용량 초과의 올바른 처방은 **장치를 끄는 것이 아니라 큐를 세우는 것**이다.
 * `DISAGREEMENT_HALT`가 그 일을 한다(server/judgmentBatch): 한 회차에서 불일치가
 * 무더기로 나면 그 자산군의 **자동 판정을 통째로 정지**시키고 P0를 올린다.
 *
...
export const CROSS_CHECK_READINESS = {
  ...
  OBSERVATION_DAYS: 7,
  /** 이 기간 동안 허용되는 어댑터 고장 건수(SOURCE_ERROR·NO_DATA) — 0이 목표다 */
