 ...
 *
 * ⚠ **왜 7인지는 기록이 없다** (2026-08-16 확인). 이 파일의 다른 상수는 전부 근거가
 * 붙어 있는데(크기 하한 k=1.2는 시뮬레이션, 이상값 필터 30%는 거래소 가격제한폭,
 * 쿨다운 48시간은 "알림을 못 본 하루를 흡수") 이 숫자만 비어 있다. 바꾸기 전에
 * 무엇이 걸려 있는지부터 재야 한다 — 지금 아는 것은 이것뿐이다:
 * 이 값을 올리면 더 많은 주식 카드가 컷오프 규칙(개장 전 창구 또는 +2일 강제)에
 * 걸리고, 내리면 그만큼 "게시 시점 기준가"로 그냥 나간다.
 */
