 ...
 *
 * ── 7의 근거 (2026-08-16 실측, scripts/simShortHorizonCutoff.ts) ──
 * "예측력 0인 사람이 게시 시각만 골라서 얻는 카드당 기대 점수"를 쟀다 — σ 추정기를
 * 고칠 때 쓴 것과 같은 잣대다(모델 p₀가 정확하면 반드시 0). 경로 30,000개 × 방향 2,
 * 장중 78틱(5분봉), σ 세 분위. **시즌 20장 누적을 시니어 승급선(1,200점)에 견준 값**:
 *
 *   시한   1일    3일    5일   **7일**   10일   14일   21일+
 *   최악  16.3%  8.8%   6.3%  **3.9%**  2.2%   1.3%   0.0%
 *
 * **7일은 "시니어선의 5% 이내"에 해당한다.** 근거 없이 고른 숫자였지만 재 보니
 * 쓸 만한 자리였다(σ 추정기를 고칠 때 문제 삼았던 수준이 22%였으니 그 1/6이다).
 *
 * 더 조이려면 14일(2%)이 다음 후보인데 **대가가 있다**: 7~13일 카드가 소급 기준가로
 * 밀려 게시 관문에서 크기 하한·방향 정합성을 검증하지 못하고 목표가형을 못 쓰게 된다.
 * 방금 개장 전 카드에서 없앤 문제를 그 띠에 다시 만드는 셈이라 **7을 유지한다.**
 *
 * ⚠ 위 수치는 **상한**이다 — 하루 78틱 중 최저점을 정확히 집어낸다고 가정했다.
 * ⚠ 21일부터 0이 되는 것은 우위가 사라져서가 아니라 **신뢰도 사다리가 성겨서**다:
