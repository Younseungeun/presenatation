 ...
 *
 * 유도 (점수 v5: S·w·ln(p̂/p₀) 또는 S·w·ln((1−p̂)/(1−p₀)), scoring.ts) —
 * 별 **한 칸**이 점수 크기를 몇 배 움직이는지(칸당 로그 기울기)를 두 축에서 잰다:
 *   · 수익성: 가중 w가 구간 1→5에서 1.00 → 2.00 → ln2를 별 칸 수로 나눈다
 *   · 신뢰도: 신고 승산이 별 1→5에서 ×1 → ×140 → ln140을 별 칸 수로 나눈다
 * 무게 = 기울기 비율. 신뢰도가 압도적으로 무거운 것이 맞다 — 수익성은 점수를 최대
 * 2배 키우지만 신뢰도는 신고 자체를 바꿔 정보량의 부호와 크기를 결정한다.
 *
