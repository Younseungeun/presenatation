...
//
// 검수 3층(학습표현·규칙·ARGOS) 사이를 항목이 오가야 하는데, "지금 옮길 때가 됐나"를
// 증거로 판단한다. 이 파일은 **추천만** 낸다 — 실제 이동은 사람 몫(개발자가 규칙 쓰거나,
// ARGOS 재학습 돌리거나). 실행 버튼은 화면에도 없다.
//
// ⚠ 문턱 숫자는 전부 **초안**이다 — 운영 실측 전이라 근거가 없다. 서버 세션 합의대로
// `@근거 설계` 초안 표식을 달고, 운영 표본이 쌓이면 재보정한다.

export type DetectionLayer = 'PHRASE' | 'RULE_WARN' | 'RULE_BLOCK' | 'ARGOS';
...
export interface DetectionItemStats {
  ...
  truePos: number;
  /** 오탐 — 걸렸는데 운영자가 "오탐"으로 승인 (위임의 이유이자 ARGOS 에게 가르칠 것) */
  falsePos: number;
  /**
  ...
  /**
   * 고유 표면형 수 (형태 안정성 판별자).
   * PHRASE 층은 등록 후 hit 의 표면형, **ARGOS 층(졸업 표현)은 졸업 후 관찰의 표면형**이다 —
  ...
  /**
   * ARGOS 동반 검출 — **layer='PHRASE'만.** 이 항목이 걸린 확정 검수 건 중 ARGOS(학생 —
   * 라이브 또는 그림자)도 같은 유형 소견을 낸 건수. 졸업(사전→ARGOS)의 실증이다:
   * 내려도 잃는 것이 없으려면 ARGOS 가 이미 잡고 있다는 영수증이 있어야 한다.
   */
  studentCoDetected?: number;
  /**
   * ARGOS 미동반 — 이 항목이 걸린 확정 건 중 ARGOS 가 같은 유형을 못 낸 건수.
   * 하나라도 있으면 사전이 하중을 지는 것이라 졸업 불가. 학생 기록이 아예 없어
   * 알 수 없는 건도 여기로 센다(모르면 내리지 않는다 — 보수 방향).
   */
  studentMissed?: number;
  /**
   * 그림자 정탐 중 **ARGOS 는 놓친** 건수 — **layer='ARGOS'(졸업 표현)만.**
   * 복귀(ARGOS→사전)의 실증이다: 옛 항목이 잡는다는 것만으로는 부족하고(ARGOS 도 잡으면
   * 중복이라 되살릴 이유가 없다), ARGOS 가 놓친 확정 위반을 옛 항목이 잡았을 때만
   * 복귀가 실제 보호 구멍을 메운다 (2026-08-31 창업자 지적).
   */
  missTruePos?: number;
  /**
   * 관찰된 표면형 예시 (빈도 상위 몇 개) — **layer='ARGOS'(졸업 표현)만.**
  ...
  /**
   * 졸업 관찰에서 ARGOS 가 놓친 **총** 횟수 — **layer='ARGOS'(졸업한 표현)만.**
   * `GraduationWatchHit.studentFlagged=false` 의 집계다 (판정 유무 무관).
   *
   * ⚠ 미탐 총수 자체는 트리거가 아니다 — ARGOS 의 실패는 재학습(수리)의 신호이고, 여기
 ...
 *  · **축 간(졸업/졸업 강등)** — 매칭 축 ↔ ARGOS(의미 추론). 상하가 아니라 **방식 교체**다:
...
export type LadderRecommendationKind =
  ...
  | 'GRADUATE_ARGOS' // [축 간 졸업] 학습표현 → ARGOS (5조건 + ARGOS 동반 검출 실증 = 중복이라 내려도 안전)
  | 'DELEGATE_ARGOS' // [축 간 졸업 — 문맥 위임] 규칙 → ARGOS (오탐 = 형태는 맞는데 문맥을 못 가름)
 ...
 *  · 위임 표본 하한 20 = ARGOS 가 무엇을 이어받는지조차 실측 못 하는 규칙은 위임감 아님
 *  · 복귀 미탐-정탐 하한 2 = "ARGOS 가 놓친 확정 위반을 옛 항목이 잡음"이 2건은 있어야
...
export const LADDER_THRESHOLDS = {
  ...
  /**
   * 졸업 추천의 ARGOS 동반 검출 최소 건수 (12차 검토 C-1 반채택, 2026-09-01) — 동반 1/1 로는
   * 우연이다. 5건은 "ARGOS 가 이 표현을 잡는다"를 패턴이라 부를 최소치(복귀 2건보다 높은 이유:
...
export function recommendMigration(
  ...
  if (s.layer === 'PHRASE') {
    ...
    // 형태가 굳었으면 규칙 승격이 우선이다 — 규칙(→BLOCK)은 ARGOS 가 절대 갖지 못하는
    // 즉시 거절로 가는 유일한 길이라, ARGOS 가 중복으로 잡고 있더라도 이 길이 더 값지다
    const formStable =
    ...
    // **졸업의 판별자는 "형태 다양"이 아니라 "ARGOS 동반 검출 실증"이다** (2026-08-31
    ...
    // 없다는 영수증**이 필요하다: 이 항목이 잡은 확정 건들에서 ARGOS 도 같은 유형을
    // 냈는가(동반 검출). 미동반이 하나라도 있으면 사전이 하중을 지고 있는 것이고,
    // 실증이 아예 없으면(동반 0·미동반 0) 모르는 것이므로 내리지 않는다.
    // 동반 1/1 은 우연이다 — 최소 건수(C-1)를 넘어야 "ARGOS 가 잡는다"가 패턴이 된다
    if ((s.studentMissed ?? 1) === 0 && (s.studentCoDetected ?? 0) >= T.graduateMinCoDetected) {
      return {
        kind: 'GRADUATE_ARGOS',
        reason:
          `5조건 통과 · ARGOS 동반 검출 ${s.studentCoDetected}건/미동반 0 — 중복 실증, ` +
          `내려도 잃는 것 없음 (사전 슬림화)`,
      };
    }
    // 형태 다양 + ARGOS 미커버(또는 미실증): 이 variation 을 잡는 유일한/실증된 층이
    // 사전이다 — 이동할 이유가 없다. 미동반 건은 ARGOS 재학습 재료가 된다
    return null;
  }

  if (s.layer === 'RULE_WARN') {
    // 오탐이 있으면 승격 자격 소멸 — 그건 "형태는 맞는데 문맥을 못 가른다" = ARGOS감.
    ...
    if (s.falsePos > 0) {
      return s.matched >= T.delegateMinMatched
        ? { kind: 'DELEGATE_ARGOS', reason: `오탐 ${s.falsePos}건 — 문맥 못 가름, ARGOS 위임(졸업) 검토` }
  ...
  if (s.layer === 'ARGOS') {
    ...
    // 가 applyRules 를 재생해 남기는 관찰)으로 "ARGOS 가 놓친 확정 위반을 옛 항목이
    // 잡는가"를 실적으로 확인한다 — 잡는 것만으로는 부족하다(ARGOS 도 잡으면 중복이다).
    //
    // **졸업 강등의 본선은 여기 없다.** 본선 = 아직 코드화 안 된 표현(ARGOS 가 잡거나
    ...
    //   · **ARGOS 는 그걸 놓쳤다** — 같은 건에서 학생이 침묵했다 (구멍 실증)
    // ARGOS 도 다 잡고 있으면(미탐 0) 옛 항목은 중복이라 되살릴 이유가 없다 — 그건
    ...
    // 재학습은 ARGOS 를 고치는 처방으로 병행된다(둘은 배타가 아니다).
    if ((s.missTruePos ?? 0) >= T.ungraduateMinMissTruePos && s.falsePos === 0) {
      ...
      return {
        kind: 'UNGRADUATE',
        reason:
          `ARGOS 가 놓친 확정 위반 ${s.missTruePos}건을 옛 사전 항목이 잡음(그림자 오탐 0` +
