...
//  ③ RESEARCHER — 리서처 자발 단축. 본인이 판매를 일찍 닫는다. **회수 불가.**
//     왜 이런 걸 원하나: 촉매형 리포트(실적 발표·이벤트 직전)는 논지가 소비되는 순간이
//     정해져 있는데, 시스템의 1/3 규칙은 그 시점을 모른다. 촉매가 지나고도 계속 팔리면
//     "이미 끝난 논지를 판 사람"이 되어 평판이 깎인다.
//     경제학적으로 이것은 **비용을 치르는 정직 신호**다 — 판매 수익을 스스로 포기하므로
//     실력 없는 사람은 흉내낼 이유가 없다. 그래서 회수를 막는다: 되돌릴 수 있으면
//     비용이 0이 되어 신호가 죽는다(닫았다 열었다 하며 희소성만 연출하게 된다).
//
...
export type SalesCloseReason = 'WINDOW_END' | 'BAND_EXIT' | 'RESEARCHER';
