...
// 정보량(c=7이면 −1.62)보다 아래로 못 간다. 당시 1단 문턱 −3.00에 닿을 방법이 없고,
// 2단(−4.61) 이하는 c=10으로도 닿지 못한다.
// (이 표는 1단 α=5%일 때의 값이다. 이후 α를 10%로 올렸지만 −2.30도 −1.62보다
//  아래라 묶음 방식의 c=7 탐지 0%는 그대로다 — scoring.DISCIPLINE_ALPHA 주석)
//
...
//   · 겹친 기간 — 카드 A의 **결과가 정해지기 전에** 카드 B를 게시했다면, B의 신고는
//     A의 결과를 모르고 이뤄진 것이다. 그것이 조건부 신고가 깨지는 자리이고,
//     얼마나 깨졌는지는 **겹친 정도**에 비례한다.
//     "결과가 정해진 시각"은 우리 배치가 돈 시각이 아니다 — 리서처는 시한의 종가로
//     스스로 안다. 그래서 closedAt은 min(판정 시각, 시한)이다 (2026-08-15)
...
export interface EvidenceCard {
  ...
  openedAt: number;
  /**
   * 카드가 닫힌 순간 (ms) — **결과가 정해진 시각이지 배치가 돈 시각이 아니다.**
   * `min(판정 시각, 검증 시한)`으로 넣는다 (scoreService 주석). 배치 실행 시각을
   * 그대로 쓰면 시세 장애·큐 정체가 겹침을 늘려 인프라 사고가 규율의 입력을 바꾼다.
   */
  closedAt: number;
 ...
 *     30장 × 90일              91% /  0% /  23%
 *
 * ⚠ **여전히 과보정이다.** ρ=1.0까지 올려도 오작동이 0.00%였다 — 보장을 지키고
 * 남는 여유가 크다는 뜻이다. 그 여유는 **1단 α를 5% → 10%로 올려** 썼다
 * (scoring.DISCIPLINE_ALPHA 주석). 하중식을 완화하는 길도 같은 지점에 닿지만
 * 보장이 구성에서 실측으로 강등되므로 고르지 않았다 — **이 파일의 ρ̄는 1로 둔다.**
 * 실제 상관을 재서 설계효과 n/(1+(n−1)ρ)를 쓰면 더 남길 수 있지만, 종목 간 상관을
 * 재려면 수익률 시계열을 저장해야 한다 — 지금은 σ만 저장한다. 이후 과제.
 *
