...
import { MARKET_SORTS, MARKET_SORT_LABEL, type MarketSort } from "@/server/marketQueries";
 ...
 * 시트만 클라이언트 상태다.
 */
export function SortPicker({ asset, sort }: { asset: string; sort: MarketSort }) {
        ...
        <>
          ...
          <div className={styles.sheet} role="dialog" aria-modal="true" aria-label="정렬 기준">
            ...
            {MARKET_SORTS.map((key) => (
              <Link
                key={key}
                href={`/leaderboard?asset=${asset}&sort=${key}`}
