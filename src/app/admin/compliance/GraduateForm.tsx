...
export function GraduateForm({
  ...
  return (
    <div className={a.branch} style={{ marginTop: 10 }}>
      ...
      {/* **졸업의 대가는 학생 상태와 무관하게 하나 더 있다** (동선 점검에서 실측).
          작성 중 검사는 규칙(코드 + 사전)만 돌고 학생은 부르지 않는다 — 디바운스 600ms 로
          사이드카를 수십 번 부를 수 없어서 의도적으로 뺐다(Q10). 그래서 졸업한 표현은
          **학생이 근무 중이어도** 작성 화면에서 영영 침묵한다.
          그림자 경고는 "당분간"이라고 말하지만 이쪽은 안 돌아온다 — 그 차이를 적지 않으면
          운영자가 "학생이 복귀하면 원래대로"라고 읽는다. 실측: 졸업 직후 같은 문장의
          작성 중 소견이 1건 → 0건 */}
      <div className={a.note}>
        <b>작성 중 경고는 영구히 사라집니다.</b> 작성 화면 검사는 규칙과 사전만 돌고 학생은
        게시 시점에만 봅니다 — 졸업하면 리서처는 <b>이 표현을 쓰는 동안 아무 말도 듣지
        못하고</b>, 제출한 뒤에야 보류를 받습니다. 사전이 원래 없애려던 낭비가 이 표현에
        한해 돌아옵니다. 그 대가를 치를 만큼 학생이 이 표현을 확실히 잡는지 보고 누르세요.
      </div>
