...
// 규칙→ARGOS 위임도 하강(▼)이 아니라 관할 이전(↗ 졸업 계열)이다 — 형태 매칭이 못 가르는
...
const REC: Record<LadderRecommendationKind, { label: string; color: string }> = {
  ...
  GRADUATE_ARGOS: { label: "↗ ARGOS 졸업 (실적)", color: "#2a6fb0" },
  DELEGATE_ARGOS: { label: "↗ ARGOS 졸업 (문맥 위임)", color: "#2a6fb0" },
  ...
  return (
    <section style={{ marginTop: 8 }}>
      <SecHead title="검출 항목 관리 — 승격·졸업 사다리">
        ...
        있을 때</b>, ↗졸업·↙졸업 강등은 <b>형태 매칭 ↔ 의미 추론(ARGOS) 중 어느 쪽이
        ...
        &ldquo;ARGOS가 이미 잡는다(중복)&rdquo;, 복귀는 &ldquo;ARGOS가 놓친 확정 위반을 옛 항목이
        잡는다(구멍)&rdquo;의 영수증이 있을 때만 뜹니다 — 그 영수증의 현재 수가{" "}
        <b>상호 실증</b> 열입니다. ARGOS 층 행의 정탐/오탐은 <b>그림자
                  ...
                  <tr key={r.id} style={{ borderTop: "1px solid var(--line)" }}>
                    <td style={{ ...td, maxWidth: 240, wordBreak: "break-word" }}>
                      ...
                      {/* 항목별 질문지 — 학습표현·규칙 WARN 만 (BLOCK 은 사람 판정이 안 붙고
                          ARGOS 는 항목 단위가 아니다). 이 항목이 잡은 문장 전부를 모아
                    ...
                    <td style={{ ...td, color: "var(--text-faint)" }}>
                      {/* 형태는 형태만 말한다 — ARGOS 행의 미탐은 상호 실증 열로 옮겼다
                          (한 칸에 형태·미탐을 겹치면 눈이 매번 다시 해석해야 한다).
                          규칙 WARN 도 출현형이 쌓이면 형태를 보인다 (12차 C-2) — 도입 전 기록뿐이면 — */}
                      {(r.layer === "PHRASE" || r.layer === "ARGOS" || r.layer === "RULE_WARN") && r.distinctSurfaces != null
                        ? `${r.distinctSurfaces}종·최빈 ${Math.round((r.topSurfaceShare ?? 0) * 100)}%`
                        : "—"}
                    </td>
                    <td style={{ ...td, color: "var(--text-faint)", whiteSpace: "nowrap" }}>
                      {r.layer === "PHRASE" ? (
                        // 졸업의 실증: 사전이 걸린 확정 건에서 ARGOS 도 같은 유형을 냈는가.
                        // 미동반 > 0 이면 사전이 아직 하중을 지고 있다 — 졸업 추천이 서지 않는다
                        // 미동반은 위험 신호가 아니다 — 사전이 켜져 있어 아무것도 새지
                        // 않았고, 다만 졸업이 아직 이르다는 뜻이라 색을 입히지 않는다
                        <span title="사전 소견이 걸린 확정 건에서 ARGOS(라이브 또는 그림자)가 같은 유형을 함께 냈는가 — 졸업(중복 실증)의 재료. 미동반이 있으면 사전이 아직 하중을 진다">
                          ARGOS 동반 {r.studentCoDetected ?? 0}·미동반 {r.studentMissed ?? 0}
                        </span>
                      ) : r.layer === "ARGOS" ? (
                        // 복귀의 실증: ARGOS 가 놓친 것 중 사람이 위반으로 확정한 부분집합.
