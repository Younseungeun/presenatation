...
import { ItemPackError, registerPhraseFromIris } from '@/server/itemTeacherPackService';
import { LearnedPhraseError } from '@/server/learnedPhraseService';
import { requireOperatorId, toErrorResponse } from '../../../_lib/http';

// 졸업 강등 본선의 실행 통로 (Q1) — IRIS 유형별 모음에서 학습 표현을 등록한다.
// 출처(그 유형의 최근 확정 IRIS 건)는 서버가 물린다 — 화면은 유형과 표현만 보낸다.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const created = await registerPhraseFromIris(prisma, { ...body, operatorUserId });
