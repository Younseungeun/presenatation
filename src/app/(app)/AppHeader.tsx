...
// - backHref를 주면 좌측이 뒤로가기 화살표가 된다. **앱 안에서 들어온 경우
//   history.back으로 진짜 이전 화면에 돌아가고**(스크롤·필터 복원), 알림·딥링크로
//   바로 진입했을 때만 backHref(논리적 부모)로 간다 — SmartBackLink.
//   left를 직접 주면 그쪽이 우선한다.
    ...
    <header className={`appbar${seamless ? " appbarSeamless" : ""}`}>
      <div className="appbarInner">
        <div className="appbarSide">
          {left ??
            (backHref ? (
              <SmartBackLink fallback={backHref} className="appbarIconBtn" aria-label="뒤로">
