...
/**
 * 간편 비밀번호 — 알약형 입력창 + 마스킹 점 넷. 자물쇠(보안)는 "닫혀 있다"이고
 * 이건 "코드를 친다"다. 둥근 사각형에서 알약(stadium)으로 바꿔 로그인 카드의 부드러운
 * 입력란에 맞췄다. 점은 넷 — 카드가 넷을 그린다.
 */
export function PinIcon() {
  return (
    <svg {...BOX}>
      ...
      />
      <g fill="currentColor">
        <circle cx="6.6" cy="12" r="1.45" />
        <circle cx="10.2" cy="12" r="1.45" />
        <circle cx="13.8" cy="12" r="1.45" />
        <circle cx="17.4" cy="12" r="1.45" />
      </g>
    </svg>
  );
}
...
/**
 * 생체 인증 — 스캔 브래킷 속 지문. 뷰파인더 네 모서리 + 돔 능선 하나 + 코어 심.
 * 2026-08-29 다시 그림. 지문만 그리면 신체 부위지 인증 행위가 아니다 — 브래킷이
 * '스캔하는 순간'을 말하고, 로그인 흐름의 얼굴 스캔 글리프와 같은 문법이라 두 생체
 * 화면이 한 언어를 쓴다. 안쪽 지문은 24px에서 살아남는 두 획(다리 달린 돔 + 심)까지만 —
 * 능선을 더 얹으면 뭉갠다. 잉크 22.2%, 최대 덩어리 48, 24px 틈 전부 생존.
 */
  ...
  return (
    <svg {...BOX}>
      <g
        ...
        strokeLinecap="round"
      >
        <path d="M2.6 8 V5.1 A2.5 2.5 0 0 1 5.1 2.6 H8" />
        <path d="M16 2.6 H18.9 A2.5 2.5 0 0 1 21.4 5.1 V8" />
        <path d="M21.4 16 V18.9 A2.5 2.5 0 0 1 18.9 21.4 H16" />
        <path d="M8 21.4 H5.1 A2.5 2.5 0 0 1 2.6 18.9 V16" />
        <path d="M8.6 16.2 V11.6 A3.4 3.4 0 0 1 15.4 11.6 V16.2" />
        <path d="M12 13.9 V8.4" />
      </g>
    </svg>
  );
}
