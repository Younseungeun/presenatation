...
export default async function Home() {
  ...
  if (userId) {
    ...
    if (user) {
      return (
        <>
          {/* ⚠ 출시 전 삭제 (2026-08-19 사용자 지시 — 개발 편의용).
              이용자 홈 ↔ 관리자 화면을 오가며 작업하기 위한 임시 버튼이다.
              운영자에게만 보이지만, 출시 빌드에는 이 블록 자체가 없어야 한다 —
              관리 화면의 존재를 이용자 화면이 광고할 이유가 없다.
              반대 방향(관리자 → 앱)은 /admin 대시보드의 "앱 화면으로" 줄이 맡는다 */}
          {user.role === "OPERATOR" && (
            <Link
              href="/admin"
              style={{
