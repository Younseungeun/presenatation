 ...
 * ⚠ **PG 는 교집합이 비어 있다 — 공식 예외 조항 (33차 JJ-3 확정)**: 명백한 위반 9문장
 *   실측에서 r5 의 PG 최고점이 정확히 현행 문장(0.839)이고 나머지는 0.50~0.83. 문장의
 *   자격 문제가 아니라 **r5 PG 헤드의 상한**이다(110M 은 전부 0.90+). 선정선 0.85 는
 *   유지한다 — 기준이 모델에 굴복하면 사후 점수 조작이다. "r5 대 PG 는 선정선 달성
 *   불가"가 기록으로 남는 것이 진실이다. 라이브 usable() 의 위반 핑 집행은 t=0.7 이라
 *   여유 0.14 로 영향 없음.
 *   반증 조건 (33차): 더 발전된 모델(300M 급 등)에서도 명백한 PG 위반 핑이 0.85 를 못
 *   넘으면 — 그때는 0.85 라는 선정선 자체가 비현실적 기준으로 증명된 것이라 하향한다.
