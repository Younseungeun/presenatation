...
function caseGuide(v: HumanVerdict | null): { headline: string; points: string[] } {
  const rejectLike = {
    ...
    points: [
      ...
      '     조건으로 적으면 규칙이 잡는가"를 여기서 정하세요.** 코드화가 존재하기 전에는',
      '     잴 것이 없어 자동 추천이 원리적으로 불가능합니다 — 이 논의가 그 공백을 메우는',
      '     장치입니다. 실행은 학습 표현 등록 또는 코드 규칙 작성이고, 어느 눈금까지',
      '     올릴지는 위 코드화 사다리 그대로입니다',
      '   · (자동 근거가 있는 유일한 지름길) **졸업했던 사전 항목의 복귀** — 그 항목은 이미',
      '     코드화돼 있어, 그림자 재생 실적(검출 항목 이력의 정탐/오탐)으로 "지금도 잡는다"를',
      '     잴 수 있습니다. 못 잡는 variation 이 남는지는 여기서 사람이 봅니다(엔진이 못',
      '     잡은 출현은 관찰에 안 남아 숫자에 안 보입니다)',
