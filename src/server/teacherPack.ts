...
function caseGuide(v: HumanVerdict | null): { headline: string; points: string[] } {
  const rejectLike = {
    ...
    points: [
      ...
      '   · **졸업 강등(IRIS→코드)** — 반대로 이 표현이 **굳은 형태로만 반복**된다면 형태',
      '     매칭의 후보입니다. 확정 기준은 **그 variation 을 코드로 정확히 서술할 수',
      '     있는가**입니다 — 형태 안정은 후보를 올리는 신호일 뿐이고, 서술 가능성은 여기서',
      '     사람이 정합니다. 내리는 것은 강등이 아니라 집행력 승급이며(즉시 거절 권한과',
      '     장애 무관은 코드뿐), 내린 뒤 어느 눈금까지 올릴지는 위 코드화 사다리 그대로입니다',
