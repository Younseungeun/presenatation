...
export async function buildTeacherPack(
  ...
  //
  // ⚠ 발화선 (2026-08-31 확정 처방): getDetectionLadder 는 판정된 검수 기록 **전체**를
  // 읽어 세므로 비용이 판정 누적 수에 비례한다. 지금은 판정이 사람 손 속도라 무해하고,
  // 관리자 화면이 이미 같은 스캔을 한다. **판정 저장이 눈에 띄게 굼떠지면** 여기를
  // "findingIds 가 든 기록만 좁혀 재계산"으로 바꿀 것 — 요약 테이블·증분 카운터는
  // 쓰지 않는다(판정 라벨은 철회·재기록으로 뒤집히는 값이라 두 번째 진실이 조용히
  // 어긋난다. 원본 재계산만이 번복을 공짜로 따라간다).
...
function ladderHistoryBlock(history: LadderHistoryLine[]): string[] {
  ...
  // ⚠ 항목을 **번호로 가리키지 않는다** — 이 절은 모든 판정 종류의 질문지에 공통인데
  // 논의 항목의 번호는 판정 종류마다 다르다(반려 계열만 3·4번이 사다리 논의다).
  // 번호는 자리를 가리켜서, 목록이 다르면 어긋나는데 말은 여전히 말이 돼 틀린 줄
  // 모른다 — 검수 층에서 "1차·2차"를 폐지한 것과 같은 이유다.
  return [
    '### 검출 항목 이력 — 사다리 논의 재료 (재학습 라벨에 넣지 마세요)',
    '',
    '이 건의 소견을 낸 항목들이 지금까지 쌓은 성적입니다. 코드화 사다리·관할 재검토를',
    '논의하게 되면 이 숫자를 근거로 써 주세요 — 정탐/오탐은 운영자 판정으로 확정된 값입니다.',
    '',
...
function caseGuide(v: HumanVerdict | null): { headline: string; points: string[] } {
  const rejectLike = {
    ...
    points: [
      ...
      '   · **졸업 강등(IRIS→코드)** — 요건은 **이 표현의 variation 전체를 코드로 정확히',
      '     서술할 수 있는가**입니다("형태가 굳었나"는 그 하위 사례일 뿐입니다). 그림자',
      '     재생 증거 — 관찰된 형태들을 기존 층이 전부 잡았고 그 문서들의 사람 판정과',
      '     일치(정탐)했는가 — 를 근거로, 못 잡는 variation 이 남는지는 여기서 사람이',
      '     봅니다(엔진이 못 잡은 출현은 관찰에 안 남아 숫자에 안 보입니다). 내리는 것은',
      '     강등이 아니라 집행력 승급이며(즉시 거절 권한과 장애 무관은 코드뿐), 내린 뒤',
      '     어느 눈금까지 올릴지는 위 코드화 사다리 그대로입니다',
      '   ⚠ **IRIS 가 놓쳤다는 사실은 이동 사유가 아닙니다** — IRIS 의 실패는 재학습으로',
      '   고치는 것이고(위 2번), 뚫리는 동안의 보호는 응급 재활성화가 맡습니다. 이동은',
      '   실패가 아니라 적합성으로만 정합니다',
    ],
  };
...
function discussionFormat(packId: string, verdict: HumanVerdict | null): string[] {
  return [
    ...
    '기록되지 않습니다.',
    '',
    // 항목을 번호("위 3·4번")로 가리키지 않는다 — 이 맺음말은 모든 판정 종류의 질문지에
    // 공통인데 번호는 판정 종류마다 다른 목록을 가리킨다 (ladderHistoryBlock 주석 참고)
    '위 결론은 재학습용 라벨일 뿐이고, 사다리 이동(학습 표현 등록·승격·졸업·졸업 강등)은',
    '창업자가 이 논의를 읽고 코드로 판단합니다 — JSON 에 넣지 마세요.',
  ];
}
