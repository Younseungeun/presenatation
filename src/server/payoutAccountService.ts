...
//   ② 변경 쿨다운 — **낯선 기기에서 바꿨을 때만** 48시간. 그 사이 본인 알림이 가서
//      "내가 안 바꿨다"고 말할 창이 생긴다. 평소 기기는 대기 0                ← 있음(기기 조건부, 2026-08-16 완화)
//   ③ 예금주명 ↔ 본인 인증 실명 대조                                    ← 있음(2026-08-16)
//   ④ **계좌 등록 시 본인 인증 재확인** — 계정만 뚫어서는 계좌를 못 바꾼다   ← 있음(2026-08-16)
//
// ── ②를 왜 기기 조건부로 완화했나 (2026-08-16 사용자 확정) ──────
// ④가 대부분의 탈취(세션·비밀번호·기기 습득)를 계좌 등록 문턱에서 끝내므로, ②까지
// 뚫고 오는 공격은 "유심 스와핑 + 본인 명의 통장"뿐이다. 그 공격자의 기기는 언제나
// **낯선 기기**다 — 그래서 낯선 기기에만 대기를 남기면 방어는 같고 정상 사용자의
// 대기만 사라진다. 고지와 대기는 같은 축(낯선 기기)에 탄다: 고지만 있고 대기가 없으면
// 이미 지급 가능한 정산이 알림 도착 전에 나가 동결 버튼이 눌러볼 새가 없다.
//
// ── ④가 ③을 실제로 작동하게 만든다 ─────────────────────────────
...
/**
 * **낯선 기기에서** 계좌를 바꾼 뒤 이만큼은 검증됐어도 돈을 안 내보낸다.
 * (평소 로그인 기기에서의 변경은 대기 없음 — 파일 머리 주석 ②)
 *
 * 길이의 근거는 **사람이 알림을 보고 반응하는 시간** — 알림을 못 본 하루를
 * 흡수하고도 하루가 남아야 한다.
 *
...
export async function registerPayoutAccount(
  ...
  input: {
    ...
    /**
     * **이 요청이 평소 로그인 기기(간편 로그인이 사는 기기)에서 왔는가** (2026-08-16
     * 사용자 확정 — 기기 조건부 쿨다운). true면 대기 없이 지급되고 고지도 없다.
     * false면 +48시간 대기 + "다른 기기에서 변경됨" 고지 — 유심 스와핑 공격자의
     * 기기는 언제나 낯선 기기라, 좁아진 위협에 방어도 좁게 맞춘 것이다.
     * 필수 인자인 이유: 선택이면 언젠가 빼먹은 경로가 전부 "평소 기기"가 된다.
     */
  ...
  // ── 고지는 **낯선 기기에서 바꿨을 때만** (2026-08-16 사용자 확정) ──────────
  //
  // 평소 로그인 기기에서의 등록·변경은 본인이 방금 한 일이라 알림이 소음이다 —
  // 알림을 아끼는 만큼 진짜 경보가 무거워진다. 이력(PayoutAccountHistory)은 기기와
  // 무관하게 언제나 남는다.
  //
  // 낯선 기기일 때의 48시간은 **골든타임**이다: 값어치는 기다림 자체가 아니라
  // 그 사이에 진짜 주인이 멈출 수 있다는 것이고, 그래서 문구가 "지급이 늦어진다"가
  ...
  if (!input.trustedDevice) {
    ...
    await prisma.notification.create({
      data: {
        ...
        title: isFirst
          ? '[확인] 정산 계좌가 등록되었습니다'
          : '[중요] 다른 기기에서 정산 계좌가 변경되었습니다',
        body: isFirst
          ? // 첫 등록은 대개 본인이 방금 한 일이다 — 다만 평소 기기가 아니라는 사실과
            // 만약을 위한 동결 안내는 남긴다: 유심을 가로챈 사람이 남의 계정에 처음
            // 계좌를 넣는 경우가 실제로 있다
...
export async function assertPayoutAccountReady(
  ...
  // 쿨다운은 **낯선 기기에서 바꾼 경우에만** 걸려 있다 (cooldownUntil — 스키마 주석).
  // 평소 기기에서의 변경은 이 칸이 null이라 대기 없이 지나간다
  if (account.cooldownUntil && now < account.cooldownUntil) {
    const hours = Math.ceil((account.cooldownUntil.getTime() - now.getTime()) / 3_600_000);
    throw new PayoutAccountError(
      `평소 기기가 아닌 곳에서 계좌를 바꾼 직후입니다 — ${hours}시간 뒤에 지급할 수 있습니다. ` +
