...
export async function consumeOperatorRecheck(
  ...
  //
  // 여기가 **1인 모드에서 돈이 나가는 모든 길이 지나는 한 지점**이다(동결 해제·고액
  // 지급·수동 판정·이의 인정·보상 실행). 그래서 이 유예를 여기 하나에만 건다 —
  // 길마다 나눠 걸면 언젠가 새 길이 생기고 그 길에는 안 걸린다. 실제로 그랬다:
  // 보상 실행이 이 점 밖에 있다가 2026-08-18 전수 점검에서 붙었다.
  //
