...
/**
 * @근거 설계 — 박동 문턱 = 주기의 3배 (회신 15호 §2). 재는 순간과 도는 순간이 어긋나기만
 *   해도 걸리면 안 되고, 재기동(배포)은 한 회차를 반드시 비운다 — 15분이면 그 안에 끝난다.
 *   세 번 연속 빠지면 타이머가 죽은 것이지 늦은 것이 아니다.
 ...
 */
export const CANARY_STALE_MS = 3 * CANARY_INTERVAL_MS;
...
export interface CanaryScreen {
  ...
  lastOkAt: Date | null;
  /** 그 박동이 문턱(주기 3배 = 15분) 넘게 낡았는가 (= 스케줄러의 카나리아 타이머 고장) */
 ...
 * 옮김 — 문턱이 15분인데 알림 주기가 하루면 28배 성기다).
 *
