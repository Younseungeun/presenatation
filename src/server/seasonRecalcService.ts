...
// - 인투빌 펠로우(상대평가)는 MVP 제외 — 재산정 대상에서 건너뛴다
// - **규율은 시즌으로 풀리지 않는다** (2026-08-13 변경). 예전에는 점수 집계가 시즌
//   단위라 새 시즌이 열리면 규율이 자동 해제됐는데, 그것이 래더를 무력화하고 있었다
//   (scoreService 주석). 이제 증거 D는 이어 쌓이고, 해제는 **적중이 쌓일 때만**
//   일어난다. 이 재산정이 건드리는 것은 등급뿐이다
