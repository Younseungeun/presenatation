...
async function stampRecheck(at: Date) {
  await prisma.user.update({ where: { id: operatorId }, data: { operatorRecheckAt: at } });
}
...
describe('생체 재확인이 두 번째 사람을 대신한다', () => {
  ...
  it('③ 낡은 재확인은 안 쳐준다 — 5분 전 지문은 지금의 확인이 아니다', async () => {
    await stampRecheck(new Date(NOW.getTime() - OPERATOR_RECHECK_WINDOW_MS - 1000));
    await expect(
      unfreezePayouts(prisma, { researcherUserId, operatorUserId: operatorId, reason: '확인' }, NOW),
    ).rejects.toMatchObject({ code: 'RECHECK_REQUIRED' });
  });

  it('방금 재확인했으면 그 자리에서 실행되고, ② 재확인은 쓰면서 사라진다', async () => {
    await stampRecheck(new Date(NOW.getTime() - 30_000));
    await unfreezePayouts(
      prisma,
      { researcherUserId, operatorUserId: operatorId, reason: '유선 본인 확인 완료' },
      NOW,
    );
    const acct = await prisma.payoutAccount.findUniqueOrThrow({ where: { researcherUserId } });
    expect(acct.frozenAt).toBeNull();
    // 도장이 지워졌다 — 다음 실행은 다시 지문을 요구한다
    const op = await prisma.user.findUniqueOrThrow({ where: { id: operatorId } });
    expect(op.operatorRecheckAt).toBeNull();
  });

  it('② 같은 재확인으로 두 번째 실행은 나가지 않는다', async () => {
    await freezePayouts(prisma, { researcherUserId, actor: researcherUserId }, NOW);
    await expect(
      unfreezePayouts(prisma, { researcherUserId, operatorUserId: operatorId, reason: '재확인' }, NOW),
    ).rejects.toMatchObject({ code: 'RECHECK_REQUIRED' });
  });
});
...
describe('④ 두 번째 진짜 운영자가 오면 2인 승인으로 돌아간다', () => {
  it('같은 실행이 이제 승인 요청을 올리고 멈춘다', async () => {
    ...
    // 생체 도장이 찍혀 있어도 소용없다 — 이제 두 번째 **사람**이 필요하다
    await stampRecheck(NOW);
    await expect(
      unfreezePayouts(prisma, { researcherUserId, operatorUserId: operatorId, reason: '확인' }, NOW),
...
describe('재확인 소비 단독 성질', () => {
  it('도장이 없으면 던진다', async () => {
    // ④ 시험이 찍고 안 쓴 도장이 남아 있다 — 지우고 잰다
    await prisma.user.update({ where: { id: operatorId }, data: { operatorRecheckAt: null } });
    await expect(consumeOperatorRecheck(prisma, operatorId, NOW)).rejects.toThrow(/지문·얼굴/);
  });
});
