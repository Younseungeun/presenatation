...
describe('배치 상호배제', () => {
  ...
  it('심박이 끊긴 락은 회수한다 — 죽은 프로세스가 배치를 영구히 잠그면 안 된다', async () => {
    const dead = new Date(Date.now() - LOCK_STALE_MS - 60_000).toISOString();
    await prisma.appSetting.create({
      data: {
        key: 'batch.lock.season',
        value: JSON.stringify({ holder: 'pid:99999', startedAt: dead, beatAt: dead }),
      },
    });
  ...
  it('심박이 도는 락은 오래돼도 뺏기지 않는다', async () => {
    const old = new Date(Date.now() - LOCK_STALE_MS - 60_000).toISOString();
    await prisma.appSetting.create({
      data: {
        key: 'batch.lock.quotes',
        value: JSON.stringify({
          holder: 'pid:12345',
          startedAt: old, // 시작은 아주 예전
          beatAt: new Date().toISOString(), // 그런데 방금 심박이 있었다
        }),
      },
    });
