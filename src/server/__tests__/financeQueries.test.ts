...
describe('getLeaderboard — 판정 기반 집계', () => {
  it('판정된 자산군만 집계: HIT +261.3, MISS −716.0 → 시즌 점수 −454.6', async () => {
    ...
    // v4: HIT +261.3 (p₀ 공제), MISS −716.0 — judgmentBatch.test와 같은 카드 사양
    expect(rows[0].seasonScore).toBeCloseTo(-454.63, 1);
...
describe('종목별 리더보드', () => {
  ...
  it('ticker를 주면 그 종목 판정만으로 집계한다', async () => {
    ...
    expect(hitOnly[0].seasonScore).toBeCloseTo(261.3, 1); // HIT 건만
    ...
    expect(missOnly[0].seasonScore).toBeCloseTo(-715.97, 1); // MISS 건만
...
describe('getAllTimeRanking — 전 기간·전 자산군 통합 랭킹', () => {
  it('자산군 구분 없이 누적 점수를 합산하고 정렬 기준을 바꿀 수 있다', async () => {
    ...
    expect(byScore[0].totalScore).toBeCloseTo(-454.6, 1);
    ...
    expect(byHitRate[0].totalScore).toBeCloseTo(-454.6, 1);
  });
});
