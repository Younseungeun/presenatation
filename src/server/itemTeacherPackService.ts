...
/** `iris:<유형>` — IRIS 유형별 모음의 항목 id 접두어. 표의 버튼이 같은 컴포넌트를 그대로 쓴다 */
export const IRIS_ITEM_PREFIX = 'iris:';

/** 이 판정들만 "확정 위반"이다 — 사다리·질문지와 같은 잣대 (MISSED = 통과 후 철회로 드러난 미탐) */
const CONFIRMED_VERDICTS = ['REJECTED', 'TAKEDOWN', 'MISSED'] as const;

/** 규칙·사전(·옛 AI 검수)이 낸 소견이 하나라도 있으면 "IRIS 만 잡은 건"이 아니다 */
function hasNonStudentFinding(findings: Finding[]): boolean {
  // source 가 없는 옛 기록은 규칙 소견으로 본다(보수 — 모르면 IRIS 몫으로 세지 않는다)
...
/**
 * IRIS 만 잡았거나 놓친 **확정** 건을 근거 문장과 함께 모은다 (2026-09-01).
 * 근거 문장(operatorEvidence)이 없는 건은 문장이 없어 재료가 못 된다 — 그래서
 * operatorVerdictWrites 가 이런 건에는 근거 문장을 **요구**한다.
 */
async function collectIrisCases(prisma: PrismaClient): Promise<IrisCase[]> {
...
export interface IrisCategoryCount {
...
export async function getIrisCategoryCounts(prisma: PrismaClient): Promise<IrisCategoryCount[]> {
  const agg = new Map<RiskCategory, IrisCategoryCount>();
  for (const c of await collectIrisCases(prisma)) {
...
async function buildIrisCategoryPack(prisma: PrismaClient, category: string) {
  ...
  const cases = (await collectIrisCases(prisma)).filter((c) => c.categories.includes(cat));
  const evidence: ItemEvidenceLine[] = cases.flatMap((c) =>
    c.evidence.map((sentence) => ({
      sentence,
      // 층 칸에 "잡았나/놓쳤나"를 싣는다 — 문장을 읽을 때 IRIS 의 현재 능력이 함께 보인다
      layer: c.detected ? 'IRIS 검출' : 'IRIS 미탐',
      verdict: 'TP' as const,
  ...
  return assembleItemTeacherPack({
    itemId: `${IRIS_ITEM_PREFIX}${cat}`,
    label: RISK_CATEGORY_LABEL[cat],
    layer: 'IRIS_CATEGORY',
    category: cat,
    stats: { matched: cases.length, truePos: cases.length, falsePos: 0, irisDetected: detected, irisMissed: cases.length - detected },
    evidence,
  });
}
...
export async function buildItemTeacherPack(
  ...
  if (itemId.startsWith(IRIS_ITEM_PREFIX)) return buildIrisCategoryPack(prisma, itemId.slice(IRIS_ITEM_PREFIX.length));
  ...
  if (row.layer !== 'PHRASE' && row.layer !== 'RULE_WARN') {
    // BLOCK 은 사람 판정이 안 붙고(즉시거절) IRIS 는 항목 단위가 아니다 — 대상 밖
