...
export const AUDIT_ACTIONS = {
  ...
  /**
   * 판정 못 한 카드의 귀책을 사람이 확정했다 (보상 대상 / 제외).
   *
   * 여기 있는 이유는 **돈의 근거가 이 판단에서 나오기 때문**이다 — 지시서를 만드는 것은
   * 배치지만 그것을 돈으로 바꾸는 것은 이 클릭이다. 제외 쪽도 똑같이 남긴다:
   * "왜 이 리서처만 못 받았나"는 지급만큼 답해야 하는 질문이다
   */
  COMPENSATION_REVIEWED: '보상 귀책 확정',
  /** 플랫폼 자본에서 보상금을 내보냈다 */
  COMPENSATION_EXECUTED: '플랫폼 귀책 보상 실행',
  /**
   * 정산 동결을 풀었다 — **운영자만 할 수 있는 일이라 여기 있다.**
   *
   * 거는 것은 누구나 할 수 있고 그건 돈을 막는 방향이라 사건이 아니다. 푸는 것은
   * **돈이 다시 나갈 수 있게 만드는 개입**이고, 하필 "본인이 아닐 수 있다"는 신고를
   * 뒤집는 일이라 누가 무엇을 확인하고 풀었는지가 남아야 한다
   */
  PAYOUT_FREEZE_SET: '정산 동결 해제',
  /**
   * 계좌 예금주명 불일치를 사람이 확인했다 — **상태는 그대로 두고 확인만 남긴다.**
   * 이름이 맞는지는 은행 조회가 답하는 것이지 운영자가 눈으로 정할 일이 아니라,
   * 여기서 상태를 열어 주면 대조 장치 자체가 없어진다
   */
  PAYOUT_ACCOUNT_MISMATCH_REVIEWED: '계좌 명의 불일치 확인',
  /**
   * 신고 보상을 **개별로 안내했다** — 지급이 아니라 안내다.
   * 쿠폰이 생기면 이 목록으로 소급 발행하므로 `rewarded`는 그대로다.
   * 손으로 하는 일이라 기록이 없으면 두 번 안내하거나 아예 빠뜨린다
   */
  ABUSE_REWARD_NOTICED: '신고 보상 안내 완료',
  ...
  /**
   * 금고 속 종이 열쇠로 비상 복구가 열렸다 (2026-08-17 검토 7차 Q1).
   *
   * **평생 한 번도 안 나오는 것이 정상인 줄이다.** 그래서 여기 있다 — 이 줄이 한 번
   * 생겼다는 사실 자체가 사건이고, 본인이 금고를 연 기억이 없다면 그 순간이 곧
   * 물리 침입 신고 시점이다. 열리는 권한은 패스키 등록 하나뿐이지만, 기록의 무게는
   * 권한의 크기가 아니라 **다시는 못 물어볼 질문**("그때 누가 금고를 열었나")에서 온다
   */
  RECOVERY_GRANTED: '비상 복구(종이 열쇠) 사용',
  /**
   * 학생 모델 자동 격하를 **재채택 증거 없이 강제로 풀었다** (11차 K-4).
   *
   * 일반 해제는 기록하지 않는다 — 증거가 맞아떨어졌다는 사실이 이미 증거다.
   * 강제 해제만 남기는 이유: 그때 시스템이 아는 것은 **사람이 그렇게 하겠다고
   * 했다**는 것뿐이라, 나중에 검수가 나빠졌을 때 "언제부터 근거 없이 돌고
   * 있었나"에 답할 수 있는 줄이 이것 하나다.
   */
  STUDENT_SHADOW_FORCE_RELEASED: '학생 모델 격하 강제 해제',
} as const;
